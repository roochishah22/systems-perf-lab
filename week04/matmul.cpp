...
int main() {
    for (int N : {1024}) {
        ...
        benchmark("naive reordered", N, [&]{ matmul_reordered(A, B, C); });
        benchmark("1 thread",  N, [&]{ matmul_parallel(A, B, C2, 1); });
        benchmark("2 threads", N, [&]{ matmul_parallel(A, B, C3, 2); });
        benchmark("4 threads", N, [&]{ matmul_parallel(A, B, C4, 4); });
        benchmark("8 threads", N, [&]{ matmul_parallel(A, B, C, 8); });
    }
    return 0;
}